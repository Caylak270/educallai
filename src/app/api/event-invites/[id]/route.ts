...
const ALLOWED = new Set(["davetli", "katildi", "iptal"]);

/** PATCH /api/event-invites/[id] — davetli katılım durumu güncelle. */
export async function PATCH(
  ...
  const { error } = await db.from("event_invites").update({ status: body.status }).eq("id", id);
